...
export function App() {
  ...
  // Обединяваме метаданните със свързването на аудиото
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col font-sans selection:bg-purple-500/30">
      <main className="flex-1 flex flex-col lg:flex-row p-4 lg:p-8 gap-8 max-w-7xl mx-auto w-full overflow-hidden">
        ...
        <div className="flex-1 flex flex-col gap-6 min-w-0">
          ...
          <div className="aspect-video bg-gray-900 rounded-3xl overflow-hidden relative">
            ...
            <div className="absolute bottom-0 w-full p-8"><AudioVisualizer analyserRef={analyserRef} isPlaying={state.isPlaying} /></div>
          </div>
      ...
      <footer className="bg-gray-900 border-t border-white/5 p-6">
        <PlayerControls
          isPlaying={state.isPlaying} isLoading={isLoading} shuffle={state.shuffle} repeatMode={state.repeatMode}
          volume={state.volume} isMuted={state.isMuted} playbackRate={state.playbackRate} currentTime={state.currentTime}
          duration={state.duration} onPlayPause={togglePlay} onPrevious={previous} onNext={next} onShuffle={toggleShuffle}
          onRepeat={toggleRepeat} onVolumeChange={setVolume} onMuteToggle={toggleMute} onPlaybackRateChange={setPlaybackRate}
          onSeek={seek} spatialMode={spatialMode} onSpatialModeChange={changeSpatialMode} isSpatialLoaded={isSpatialLoaded}
        />
      </footer>
